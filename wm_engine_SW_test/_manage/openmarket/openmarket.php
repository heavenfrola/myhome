...
<form method="post" enctype="multipart/form-data" action="./index.php" target="hidden<?=$now?>">
	...
	<table class="tbl_row multi_shop">
		...
		<tr>
			...
			<td>
				<label><input type="radio" name="openmarket_scrap_order" value="2" <?=checked($cfg['openmarket_scrap_order'], 2)?>> 입금완료</label>
				<label><input type="radio" name="openmarket_scrap_order" value="40" <?=checked($cfg['openmarket_scrap_order'], 40)?>> 등록대기</label>
				<ul class="list_msg">
		...
		<tr>
			...
			<td>
				<label><input type="radio" name="openmarket_dlv" value="N" <?=checked($cfg['openmarket_dlv'], 'N')?>> 연동안함</label>
				<label><input type="radio" name="openmarket_dlv" value="Y" <?=checked($cfg['openmarket_dlv'], 'Y')?>> 즉시연동</label>
				<ul class="list_msg">
...
<form method="post" enctype="multipart/form-data" action="./index.php" target="hidden<?=$now?>">
	...
	<table class="tbl_col multi_shop">
		...
		<tbody>
			<?while($data = parseOpenmarket($res)) {?>
			<tr>
				<td><?=($opmkey+1)?></td>
				<td>

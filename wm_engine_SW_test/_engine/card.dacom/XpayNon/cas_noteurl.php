...
$ono = addslashes($LGD_OID);
if(!$ono) exit('필수값이 없습니다.');
$ord = $pdo->assoc("select ono, pay_type, pay_prc, date1, stat, mobile, buyer_name, buyer_cell from $tbl[order] where ono='$ono'");
...
$LGD_MERTKEY = ($ord['mobile']=='Y') ? $cfg['card_mobile_dacom_key'] : $cfg['card_dacom_key'];//LG유플러스에서 발급한 상점키로 변경해 주시기 바랍니다.

$LGD_HASHDATA2 = md5($LGD_MID.$LGD_OID.$ord['pay_prc'].$LGD_RESPCODE.$LGD_TIMESTAMP.$LGD_MERTKEY);
...
if( $LGD_HASHDATA2==$LGD_HASHDATA) { //해쉬값 검증이 성공이면
	if ( "0000" == $LGD_RESPCODE ) { //결제가 성공이면
		if("I"==$LGD_CASFLAG) {
			...
			if($ord['stat'] != 1 && $ord['stat'] != 2) exit('OK');
			...
			ordStatLogw($ono, 11, 'Y');
			makeOrderLog($ono);

			$resultMSG = "OK";
		}
		else {
			// LGD_CASFLAG==R(무통장정보 할당) 또는 기타결제
			$resultMSG = "OK";
		}
	}
	else { //결제가 실패이면

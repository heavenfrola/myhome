...
nhn.HuskyRange = jindo.$Class({
	...
	*/
	...
	delFILE : function(no){ 
		...
		alert(this.getRawContents());
		alert(this.sCurrentRawContents);
	},
...
//	iMinUndoInterval : 50,
	...
	$init : function(iframe){
		...
		this.sBlankPageURL = root_url+"/main/exec.php?exec_file=smartEditor/smart_editor2_inputarea.html";
		this.sBlankPageURL_EmulateIE7 = root_url+"/_data/smartEditor/smart_editor2_inputarea_ie8.html";
	...
	$BEFORE_DEL_FILE : function(){
		if(this.oApp.getEditingMode() !== this.sMode){
			this.oApp.exec("CHANGE_EDITING_MODE", [this.sMode]);
		}
	},
	
	$ON_DEL_FILE : function(no, oPSelection, bNoUndo){
...
nhn.husky.SE2M_FontNameWithLayerUI = jindo.$Class({
	...
	$ON_SET_FONTFAMILY : function(sFontFamily, sDefaultSize){
		...
		if (typeof toggleFontControl == 'function') {
			toggleFontControl();
		}
	},
...
nhn.husky.SE2B_CSSLoader = jindo.$Class({
	...
	*/
	...
	loadSE2CSS : function(sMsg, oArgs){
		...
		nhn.husky.SE2M_Utils.loadCSS(this.htOptions.sCSSBaseURI+"/smart_editor2_items"+(typeof parent.browser_type != 'undefined' && parent.browser_type == 'mobile' ? '_m' : '')+".css", fnCallback);

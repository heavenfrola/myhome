<?
	...
	$g_conf_gw_url    = ($cfg['card_mobile_test'] == '_test') ? "testpaygw.kcp.co.kr" : "paygw.kcp.co.kr";
    $g_conf_site_cd   = ($cfg['card_mobile_test'] == '_test') ? 'T0000':$cfg['card_mobile_site_cd'];
    $g_conf_site_key  = ($cfg['card_mobile_test'] == '_test') ? '3grptw1.zW0GSo4PQdaGvsF__':$cfg['card_mobile_site_key'];
    $g_conf_site_name = ($cfg['card_mobile_test'] == '_test') ? 'KCP TEST SHOP':$cfg['card_mobile_site_name'];
	$g_conf_home_dir    = $engine_dir."/_engine/card.kcp/smartPay";   // BIN 절대경로 입력
	$g_conf_gw_port   = "8090";        // 포트번호(변경불가)

    require $engine_dir."/_engine/card.kcp/smartPay/pp_ax_hub_lib.php";              // library [수정불가]
   ...
   if ( $req_tx == "pay" )
    ...
    else if ( $req_tx == "mod" )
    {
		...
		if($mod_type == "STPA" || $mod_type == "RN07") // 부분매입취소의 경우
		 {
	...
	if($req_tx == 'mod') {
		$output_vals=array("cno", "res_cd", "res_msg", "mod_mny", "ordr_idxx");

		foreach($output_vals as $ket=>$val){
			$value = ($_POST[$val]) ? $_POST[$val] : ${$val};
			echo $val.":".$value.";";
		}
		exit();
	}
...
<? if($tnarla) { ?>
    ...
    <form name="pay_info" method="post" action="<?=$result_page?>">

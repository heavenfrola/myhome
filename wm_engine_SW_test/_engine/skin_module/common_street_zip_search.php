<?
	...
	<input type=\"hidden\" name=\"zip_mode\" value=\"2\">
	<input type=\"hidden\" name=\"urlfix\" value='Y'>";

	if($search){
        if ($_GET['form_nm'] == 'ordFrm') {
            $cartres = checkDeliveryRangeList();
        }

		$_tmp = "";
		$_line = getModuleContent("common_street_zipcode_list");
		//행자부 주소API 사용
		if($cfg['juso_api_use'] == "Y") {
			if($res && is_array($res)){
				foreach ($res as $data) {
					$data['zipcode'] = $data['zipNo'];
					$data['address'] = str_replace($search,"<b><FONT COLOR='#0033FF'>".$search."</font></b>",$data['roadAddr']);
					$data['address2'] = $data['roadAddr'];
					...
					$_tmp .= lineValues("common_street_zipcode_list", $_line, $data);
				}
			}
		} else {
		...
		$_replace_code[$_file_name]['common_street_zipcode_list'] = $_tmp;
	}

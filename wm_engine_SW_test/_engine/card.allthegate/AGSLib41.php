...
* @param paramNames -  파라미터 변수명 배열 => {상점아이디, 주문번호, 금액, 상점에서 추가한 파라미터}
* @param paramValues -파라미터에 1:1 대응되는 값들
*/
	function checkConfig($paramNames, $paramValues)
	{
		$list = array();
		$configVars = parse_ini_file($this->REQUEST["store_propPath"], TRUE);
		array_push($list , $configVars["config"]["USE_SESSION_YN"]);
		array_push($list , $configVars["config"]["USE_DB_YN"]);
		array_push($list , $configVars["config"]["LIMITRATE"]);
		array_push($list , $configVars["config"]["MIN_WON"]);

		$use_session = $list[0];
		$use_db = $list[1];

		/**************************************************
		 *  session을 사용하는 경우에만 아래 사항 적용
		 **************************************************/
		if($use_session = "Y") {
			// 파라미터 명,값 정합 체크
			if(count($paramNames) != count($paramValues)) {
				print "파라미터 이름과 값의 갯수가 다릅니다.";
			}
			// 세션에 저장
			$_SESSION['paramNames']  = $paramNames;
			$_SESSION['paramValues'] = $paramValues;
		}
	 ...
	 * 결제시에 Session의 값과 비교
	 * @param req - request 객체
	 * @param paramValues - StoreId, OrdNo, Amt, 기타 필요 시 추가 요망
	 * @return int result - 0:정상(거래가능), 1:위변조된 금액으로 의심(거래가능), 2:거래금액이 상점에서 설정한 값보다 작은 경우(거래 불가)
	 */
	 function compareValue($paramValues, $whereClause){
		/*
			Log 기록 객체생성
   		*/
		$this->log = new PayLog( $this->REQUEST );
		...
		// 결제금액비교성공(session)
		$SESSION_SUCCESS      = "S010";
		// 금액하안비율error(session)
		$SESSION_FAIL_RATE    = "S011";

		// 최소금액 error(session)
		$SESSION_FAIL_MIN     = "E911";
		// SESSION 값 NULL(session)
		$SESSION_FAIL_NULL    = "E912";
		// 결제금액비교실패(session)
		$SESSION_FAIL_COMPARE = "E919";
		...
		$DB_FAIL_NULL    = "E952";
		// 결제금액비교실패(DB)
		$DB_FAIL_COMPARE = "E959";

		// DB error(DB)
		$DB_ERROR = "E979";

		// 프로퍼티나 파일에 SESSION이나 DB 둘다 Y로 설정하지 않은 경우
		$NOT_SETTING = "E981";
		// 프로퍼티나 파일에 SESSION이나 DB 둘다 Y로 설정한 경우
		$BOTH_SETTING = "E982";

		// 프로퍼티 설정값과 파라미터 값이 매칭되지 않는경우,  ex> DB 사용중이면서, where 조건절 파라미터의 배열이 null 인 경우,
		$MISMATCH_PROP_PARAM = "E983";
		// 값에 문자열이 있는 경우
		$NOT_NUMBER = "E984";
		...
		array_push($list , $configVars["config"]["LIMITRATE"]);
		array_push($list , $configVars["config"]["MIN_WON"]);
		array_push($list , $configVars["config"]["DB_DRIVER"]);
		array_push($list , $configVars["config"]["DB_HOST"]);
		array_push($list , $configVars["config"]["DB_ID"]);
		array_push($list , $configVars["config"]["DB_PW"]);
		array_push($list , $configVars["config"]["DB_DATABASE"]);
		array_push($list , $configVars["config"]["SQL"]);
		array_push($list , $configVars["config"]["SQLCNT"]);

		if($list[0] =="Y" && $list[1] =="Y"){
			return $BOTH_SETTING;
		}else if($list[0] =="N" && $list[1] =="N"){
			return $NOT_SETTING;
		}else if($list[0] =="Y" && $list[1] =="N"){
			$limitrate = $list[2];
			$amt = $paramValues[2];
			$min_won_num = $list[3];

			// 금액이 최소 설정 금액보다 작을 경우,
			if($amt < $min_won_num) {
				$this->log->WriteLog( WARN, "주문 금액이 설정된 최소 금액보다 작습니다. 주문금액 : ".$amt.", 설정된 주문 최소 금액 : ".$min_won_num);
				return $SESSION_FAIL_MIN;
			}


			if(count($_SESSION['paramValues']) != count($paramValues)) {
				$this->log->WriteLog( WARN, "세션이 만료 되었습니다.");
				return $SESSION_FAIL_NULL;
			}

			if(count($_SESSION['paramValues']) != count($paramValues)) {
				$this->log->WriteLog( WARN, "파라미터의 갯수가 다릅니다.");
			}

			$ori_amt = $_SESSION['paramValues'][2];
			if(($this -> IsNumber($ori_amt)) == false){
				return $NOT_NUMBER;
			}

			// 결제 금액에 설정된 비율보다 작을 경우,
			if($amt < ($ori_amt*$limitrate)/100) {
				logger.warn("주문 금액이 설정된 금액 비율보다 작습니다. 주문금액 : " + $amt + ", 설정된 비율 금액 : " + (($ori_amt*$limitrate)/100));
				$this->log->WriteLog( WARN, "주문 금액이 설정된 금액 비율보다 작습니다. 주문금액 : " .$amt.", 설정된 비율 금액 : " .(($ori_amt*$limitrate)/100));
				return $SESSION_FAIL_RATE;
			}
			return $SESSION_SUCCESS;
		}else if($list[0] =="N" && $list[1] =="Y"){

			if($whereClause == "") {
				return MISMATCH_PROP_PARAM;
			}

			for($i=0;$i<$list[10];$i++){
				$list[9] = preg_replace("/\?/", "='".$whereClause[$i]."'", $list[9], 1);
			}
			...
			//사용할 oracle 관련 셋팅
                ...
                global $pdo;
                if($pdo instanceof PDODatabase) {
                    $rows = $this->assoc($sql);
                }

				$org_won = $rows["amt"];
				$min_won = $list[3];
				$req_amt = $paramValues[2];

				if($req_amt < $min_won) {
					$rtnMsg = $DB_FAIL_MIN;
				}

				if($paramValues[2] == $org_won) {
					// 소켓으로 상점 정보 전송
					$this->sendAgs($paramValues[0]);
					return $DB_SUCCESS;
				} else {
					return $DB_FAIL_COMPARE;
				}
	...
	/**
	 * 전문에 금액 위변조 관련 여부를 첨부하기 위해 문자열 생성
	 * @param userEmail - 주문자 Email
	 * @param compareResult - 금액위변조 체크한 결과값(0,1,2)
	 * @return 금액위변조 관련 여부 체크된 값 추가된 문자열
	 */
	 function makeUserEmail($userEmail, $compareResult){
		$list = array();
		$configVars = parse_ini_file($this->REQUEST["store_propPath"], TRUE);

		$use_session_yn = $configVars["config"]["USE_SESSION_YN"];
		$use_db_yn      = $configVars["config"]["USE_DB_YN"];
		$min_won        = $configVars["config"]["LIMITRATE"];
		$limitrate      = $configVars["config"]["MIN_WON"];

		$session_paramValues = array($_SESSION['paramValues']);
		$org_won = $_SESSION['paramValues'][2];

		$check_won = "Y";
		if($use_session_yn == "N" && $use_db_yn == "N") {
			$check_won = "N";
		}
		$min_yn = "Y";
		if($limitrate =="100") {
		...
		$sb = $userEmail.";".$org_won.";".$check_won.";".$min_yn.";".$limitrate.";".$min_won.";".$successyn.";";
		return trim($sb);
	 }

 	function sendAgs($service_id){
		$send_dt = date("Ymd");
		$sent_dt = $this -> getAgsValueByKey();
		...
		$gubun ="000108D";
		$sendMsg  = $this -> getFixedLengthText($gubun       , 7  , " ");
		$sendMsg .= $this -> getFixedLengthText($service_id  , 20 , " ");
		$sendMsg .= $this -> getFixedLengthText($send_dt     , 8  , " ");
		$sendMsg .= $this -> getFixedLengthText("p"			 , 1  , " ");
		$sendMsg .= $this -> getFixedLengthText("null"		 , 20 , " ");
		$sendMsg .= $this -> getFixedLengthText("PHP"		 , 20 , " ");
		$sendMsg .= $this -> getFixedLengthText($chk_won     , 1  , " ");
		$sendMsg .= $this -> getFixedLengthText($min_yn      , 1  , " ");
		$sendMsg .= $this -> getFixedLengthText($limitrate   , 3  , "0");
		$sendMsg .= $this -> getFixedLengthText($min_won     , 12 , "0");
		$sendMsg .= $this -> getFixedLengthText("null"		 , 20 , " ");
		$sendMsg .= "\r";
		...
		/** 소켓 open **/
		$this->fp = fsockopen( $addr, $port , $errno, $errstr, $conn_timeout );

		if(!$this->fp){fclose($this->fp);}
		/** 연결에 성공하였으므로 데이터를 받는다. **/
		fputs( $this->fp, $sendMsg );
		socket_set_timeout($this->fp, $read_timeout);
		$rs_code = fgets($this->fp, 25);

		/** 소켓 close **/
		fclose( $this->fp );

		$this -> setAgsProperty($send_dt);
		return;
	}

	function getAgsValueByKey() {
		$list = array();
		$configVars = parse_ini_file($this->REQUEST["ags_propPath"], TRUE);
		array_push($list , $configVars["config"]["DATE"]);
		$msg = $list[0];
		return $msg;
	}

	function setAgsProperty($send_dt) {
		$propPath;
		$msg;

		// 프로퍼티 파일 classpath 상대 경로
		$propPath= $this->REQUEST["ags_propPath"];
		$fh = fopen($propPath, 'w');
		fwrite($fh, "[config]\r\nDATE=".$send_dt);
		fclose($fh);
	...
	function getFixedLengthText($oriStr, $fixedLength, $fillStr) {
		$convertText = "";

		if(strlen($oriStr) < $fixedLength) {
			// 숫자일 경우, 앞에서부터0을 채우고 오른쪽끝에 숫자
			if($fillStr == "0"){
				for($i=0; $i<$fixedLength-strlen($oriStr); $i++){
					$convertText .= $fillStr;
				}
				$convertText .= $oriStr;
			// 문자열 일 경우, 왼쪽 정렬 후 나머지 공백으로 채움
			} else if($fillStr == " ") {
				$convertText = $oriStr;
				for($i=0; $i<$fixedLength-strlen($oriStr); $i++) {
					$convertText .= $fillStr;
		...
		} else if(strlen($oriStr) > $fixedLength) {
			$convertText = SUBSTR($oriStr, 0, strlen($convertText));
		} else {
			return $oriStr;
		}

...
?>
<script type='text/javascript' src='//www.allthegate.com/plugin/AGSWallet_utf8_New.js'></script>
<script type='text/javascript'>
function Pay(form) {
	MakePayMessage(form);
}
</script>
<form id='frmAGS_pay' name='frmAGS_pay' style="display:none" method='post' action='/main/exec.php' target='hidden<?=$now?>'>
	<input type='hidden' name='exec_file' value='card.allthegate/card_pay.exe.php' />

	<!-- 필수값 -->
	<input type='hidden' name='StoreId' value='' />		<!-- 상점 아이디 -->
	...
	<input type='hidden' name='Job' value='' />			<!-- 지불방법 -->
	<input type='hidden' name='UserId' value='' />		<!-- 주문자 회원아이디 -->
	<input type='hidden' name='UserEmail' value='' />	<!-- 주문자 이메일 -->
	<input type='hidden' name='MallUrl' value='' />		<!-- 판매자 URL -->
	<input type='hidden' name='OrdNm' value='' />		<!-- 주문자명 -->
	<input type='hidden' name='OrdAddr' value='' />		<!-- 주문자 주소 -->
	<input type='hidden' name='OrdPhone' value='' />	<!-- 주문자 연락처 -->

	<!-- 카드결제 선택값 -->
	<input type='hidden' name='QuotaInf' value='0'>		<!-- 할부기간 설정 : 구분 -->

	<!-- 가상계좌 필수값 -->
	<input type='hidden' name='MallPage' value='' />	<!-- 납부통보 페이지 -->

	<!-- 선택값 -->
	<input type='hidden' name='RcpNm' value='' />		<!-- 수신자명 -->
	<input type='hidden' name='RcpPhone' value='' />	<!-- 수신자 연락처 -->
	<input type='hidden' name='DlvAddr' value='' />		<!-- 배송지 -->
	<input type='hidden' name='Remark' value='' />		<!-- 비고 -->
	<input type='hidden' name='CardSelect' value='' />	<!-- 카드사 선택 -->
	<input type='hidden' name='HP_ID' value='' />		<!-- 주문자 연락처 -->
	<input type='hidden' name='HP_PWD' value='' />		<!-- 주문자 연락처 -->
	<input type='hidden' name='HP_UNITType' value='1' /><!-- 주문자 연락처 -->
	<input type='hidden' name='ProdCode' value='' />	<!-- 상품코드 -->
	<input type='hidden' name='HP_SUBID' value='' />	<!-- 주문자 연락처 -->
	...
	<input type=hidden name=VIRTUAL_DEPODT value="<?=$VIRTUAL_DEPODT?>">    <!-- 입금기한 -->
